...
public class CommandEnvelope<T> {
    private final String commandId = UUID.randomUUID().toString(); // 커맨드 고유 ID
    ...
    private final String correlationId;    // Saga 추적을 위한 ID
    private final long timestamp = System.currentTimeMillis(); // 커맨드 발행 시간

    public CommandEnvelope(String commandType, String targetAggregateId, T payload, String correlationId) {
